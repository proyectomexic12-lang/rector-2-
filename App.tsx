...
import { GraduationCap, Loader2, AlertTriangle, LogOut, User as UserIcon, Shield, LayoutDashboard, Database, Sparkles, ShieldAlert, Upload, MessageCircle, ShieldCheck, MessageSquare, BookOpen, FileCheck2, Settings, Award, CreditCard } from 'lucide-react';
...
function App() {
  ...
  const handleGenerate = async (refinementConfig?: { instruction: string }) => {
    ...
    // VALIDACIÓN DE CUOTA Y PLAN (20 Planeaciones / Mes, 60 / Trimestre, o Créditos Básicos)
    if (currentUser) {
      try {
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-outfit pb-20 relative selection:bg-blue-100 selection:text-blue-900">
                  ...
                  >
                    ...
                    <div className="flex flex-col leading-none">
                      ...
                      <span className="text-[9px] text-slate-400 font-medium">
                        {userQuota.maxQuota >= 60 ? 'Plan 3M' : 'Plan Mes'}
                      </span>
                    </div>
                  </button>
                ) : (
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-20">
            ...
            {activeTab === 'create' && (
              <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-6">
                      <div className="p-4 rounded-xl bg-slate-950/60 border border-slate-800 space-y-2">
                        ...
                        <p className="text-[11px] text-slate-400">
                          20 Planeaciones Didácticas completas por ciclo de 30 días.
                        </p>
                      </div>
                      ...
                      <div className="p-4 rounded-xl bg-slate-950/60 border border-indigo-500/40 space-y-2 relative">
                        ...
                        <p className="text-[11px] text-slate-400">
                          60 Planeaciones Didácticas (Ahorro directo de $10.000 COP).
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <strong className="text-white font-mono text-sm tracking-wider">3205957019</strong>
                        </div>
                      </div>
                ...
                {currentUser && !userQuota?.hasPlan && !(currentUser.email || '').toLowerCase().includes('alex') && (
                  ...
                  <div className="max-w-4xl mx-auto mb-8 bg-gradient-to-br from-amber-950/70 via-slate-900 to-indigo-950/70 border-2 border-amber-500/60 rounded-3xl p-6 sm:p-8 text-slate-200 shadow-2xl relative overflow-hidden backdrop-blur-xl">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 pb-6 border-b border-amber-500/20">
                      <div className="space-y-2">
                        <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[11px] font-black tracking-wider uppercase ${
                          creditsLeft === 0 
                            ? 'bg-red-500/20 border border-red-500/40 text-red-300' 
                            : 'bg-amber-500/20 border border-amber-500/40 text-amber-300'
                        }`}>
                          {creditsLeft === 0 
                            ? '🛑 Créditos de Cortesía Agotados (0 / 3)' 
                            : '🎁 3 Planeaciones Didácticas de Cortesía Asignadas'}
                        </div>
                        <h3 className="text-xl sm:text-2xl font-black tracking-tight text-white flex items-center gap-2.5">
                          <AlertTriangle className={creditsLeft === 0 ? "text-red-400 shrink-0" : "text-amber-400 shrink-0"} size={24} />
                          {creditsLeft === 0 
                            ? 'Cupo Agotado • Pago Inmediato Requerido' 
                            : 'Renovación de Suscripción Requerida'}
                        </h3>
                        <p className="text-xs sm:text-sm text-slate-300 leading-relaxed max-w-2xl">
                          {creditsLeft === 0 ? (
                            <>Has completado tus <strong className="text-amber-300 font-bold">3 planeaciones de cortesía</strong>. Para continuar generando secuencias didácticas y desbloquear el sistema, debes cancelar tu suscripción mensual o trimestral.</>
                          ) : (
                            <>Has completado el cupo de tus planeaciones anteriores. Te hemos habilitado <strong className="text-amber-300 font-bold">3 créditos de cortesía</strong> para que continúes sin frenar tus clases mientras renuevas tu plan.</>
                          )}
                        </p>
                      </div>

                      <div className={`border rounded-2xl p-4 text-center shrink-0 w-full md:w-auto ${
                        creditsLeft === 0
                          ? 'bg-red-500/10 border-red-500/30'
                          : 'bg-amber-500/10 border-amber-500/30'
                      }`}>
                        <span className="text-[10px] uppercase font-black text-amber-300 tracking-wider block mb-1">Disponibles Ahora</span>
                        <div className={`text-3xl font-black font-mono ${creditsLeft === 0 ? 'text-red-400' : 'text-white'}`}>
                          {creditsLeft ?? 3} / 3
                        </div>
                        <span className="text-[10px] text-amber-200/80 font-semibold">
                          {creditsLeft === 0 ? 'Créditos Consumidos' : 'Créditos de Cortesía'}
                        </span>
                      </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-6">
                      <div className="p-4 rounded-2xl bg-slate-950/80 border border-indigo-500/40 space-y-2 relative overflow-hidden">
                        <div className="flex justify-between items-center text-xs">
                          <span className="font-bold text-indigo-300 flex items-center gap-1.5">
                            <Award size={15} className="text-indigo-400" /> Plan Trimestral (Recomendado)
                          </span>
                          <span className="font-extrabold text-emerald-400 text-sm sm:text-base">$35.000 COP</span>
                        </div>
                        <p className="text-[11px] text-slate-300 leading-relaxed">
                          <strong className="text-white">60 Planeaciones Didácticas:</strong> Te aconsejamos este plan para terminar todo el periodo escolar sin preocuparte por recargas mensuales.
                        </p>
                      </div>
                      ...
                      <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 space-y-2">
                        <div className="flex justify-between items-center text-xs">
                          <span className="font-semibold text-slate-300">Plan Mensual</span>
                          <span className="font-bold text-white text-sm sm:text-base">$15.000 COP</span>
                        </div>
                    ...
                    <div className="p-4 rounded-2xl bg-slate-950/90 border border-amber-500/30 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-amber-500/20 border border-amber-500/30 flex items-center justify-center text-amber-400 shrink-0">
                          <CreditCard size={20} />
                        </div>
                        <div>
                          <span className="text-slate-400 block text-[11px]">Canal Oficial de Pago (Nequi / Transferencia):</span>
                          <strong className="text-amber-300 font-mono text-base tracking-wider">3205957019</strong>
                        </div>
                      </div>

                      <div className="text-[11px] text-slate-300 font-medium text-center sm:text-right">
                        <a
                          href={`https://wa.me/573107304533?text=${encodeURIComponent('Hola Administrador, adjunto mi comprobante de pago Nequi para la activación de mi suscripción.')}`}
                          target="_blank"
                          rel="noopener noreferrer"
                        ...
                        >
                          Mandar comprobante WhatsApp (3107304533)
                        </a>
                      </div>
                    </div>
                  </div>
        ...
        {currentUser && currentUser.role !== 'admin' && authService.isUserUnlimited(currentUser) && (
          <a
            href="https://wa.me/573107304533?text=Hola,%20soy%20usuario%20Premium%20de%20la%20plataforma%20Guaimaral%20AI.%20Necesito%20soporte%20o%20tengo%20una%20sugerencia:"
            target="_blank"
            rel="noopener noreferrer"
