...
export const UserManagement: React.FC = () => {
    ...
    const handleSelectMultigrado = () => {
        const multiGrados = GRADOS.filter(g => g.toLowerCase().includes('multi'));
        setTempGrados(multiGrados);
    };
                                            ...
                                            {subStatus.status === 'admin' ? (
                                                ...
                                                <div className="flex flex-col items-center gap-0.5">
                                                    <span className={`inline-flex items-center gap-1 text-[10px] font-black px-2.5 py-1 rounded-full border shadow-sm ${
                                                        ...
                                                        {user.stats?.cycleUsage || 0} / {user.stats?.maxQuota || (subStatus.monthsPaid >= 6 ? 120 : subStatus.monthsPaid >= 3 ? 60 : 20)} planeaciones
                                                    </span>
                        ...
                        {activeModalTab === 'areas' || activeModalTab === 'grados' ? (
                            <div className="px-8 py-3 border-b border-slate-100 flex items-center justify-between shrink-0 bg-slate-50">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    {activeModalTab === 'grados' && (
                                        <button onClick={handleSelectMultigrado} className="text-[9px] font-black text-emerald-700 uppercase bg-emerald-50 hover:bg-emerald-100 px-3 py-1.5 rounded-lg transition-all flex items-center gap-1">🏡 Multigrados</button>
                                    )}
                            ...
                            {activeModalTab === 'areas' || activeModalTab === 'grados' ? (
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                    {(activeModalTab === 'areas' ? AREAS : GRADOS).map((item) => {
                                        ...
                                        const isMulti = activeModalTab === 'grados' && item.toLowerCase().includes('multi');
                                        return (
                                            <button
                                                key={item}
                                                onClick={() => activeModalTab === 'areas' ? toggleArea(item) : toggleGrado(item)}
                                                className={`flex items-center justify-between gap-3 p-4 rounded-2xl border-2 transition-all text-left ${isSelected
                                                    ? isMulti
                                            ...
                                            >
                                                <div className="flex items-center gap-3 min-w-0">
                                                    <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center shrink-0 transition-all ${isSelected
                                                        ? isMulti ? 'bg-emerald-600 border-emerald-600' : 'bg-blue-600 border-blue-600'
                                                ...
                                                {isMulti && (
                                                    <span className="shrink-0 text-[8px] font-extrabold uppercase px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-800 tracking-wider">
                                                        Rural
                                                    </span>
                                                )}
                                            </button>
                                            ...
                                            <div className="mt-5 pt-4 border-t border-emerald-200/80 space-y-4">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                                                        {[
                                                            { months: 1, price: 15000, label: '1 Mes', quota: '20 Planeaciones', badge: 'Mensual' },
                                                            { months: 3, price: 35000, label: '3 Meses', quota: '60 Planeaciones', badge: 'Oferta 35k' },
                                                            { months: 6, price: 75000, label: '6 Meses', quota: '120 Planeaciones', badge: 'Semestral' },
                                                        ].map(plan => {
