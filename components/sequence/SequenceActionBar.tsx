...
import { Printer, FileDown, BookOpen, GraduationCap, Copy, ArrowLeft, FileText, ChevronDown, Send, CheckCircle } from 'lucide-react';
import { generateDocx } from '../../services/docxService';
import { DidacticSequence, SequenceInput } from '../../types';
import { EnviarRevisionModal } from './EnviarRevisionModal';
...
export const SequenceActionBar: React.FC<SequenceActionBarProps> = ({
  ...
  return (
    <>
      <div className="bg-slate-900/90 backdrop-blur-xl sticky top-4 z-50 p-2.5 sm:p-3 rounded-2xl shadow-2xl border border-white/10 mb-6 no-print flex flex-col md:flex-row items-center justify-between gap-3 text-white transition-all">
        ...
        <div className="bg-slate-800/90 p-1 rounded-xl border border-slate-700/60 flex items-center gap-1 w-full md:w-auto justify-center">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-black transition-all ${activeView === 'docente'
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-black transition-all ${activeView === 'estudiante'
        ...
        <div className="flex items-center gap-2 flex-wrap w-full md:w-auto justify-end">
          {/* Botón Registrar Planeación en SIGEP */}
          {editableData && (
            <button
              onClick={() => setShowRevisionModal(true)}
              className="px-3.5 py-2.5 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white text-xs font-black rounded-xl transition-all flex items-center gap-1.5 shadow-lg shadow-indigo-600/30 hover:scale-105 active:scale-95"
              title="Registrar y enviar planeación a SIGEP / Coordinación"
            >
              <Send size={14} />
      ...
      {/* Modal de Envío a Revisión Institucional */}
      {editableData && (
        <EnviarRevisionModal
          isOpen={showRevisionModal}
          onClose={() => setShowRevisionModal(false)}
          secuencia={editableData}
          input={input}
          onSuccess={(msg) => {
            setSentSuccess(true);
